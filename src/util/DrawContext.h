...
public:
    ...
    virtual void fillTriangle(Vec2 const& a, Vec2 const& b, Vec2 const& c, d2d::Color const& col) = 0;
    ...
    virtual void drawText(RectF const& rc, std::wstring const& text, d2d::Color const& color,
                          ...
                          bool hyphen = false, bool wrap = false) = 0;
    ...
    virtual Vec2 getTextSize(std::wstring const& text, Renderer::FontSelection font, float size,
                             ...
                             std::optional<Vec2> bounds = std::nullopt, bool wrap = false) = 0;
    /// Be careful when setting cache to true
...
public:
    ...
    virtual void fillTriangle(Vec2 const& a, Vec2 const& b, Vec2 const& c, d2d::Color const& col) override;
    virtual void drawText(RectF const& rc, std::wstring const& text, d2d::Color const& color,
                          Renderer::FontSelection font, float size = 30.f,
                          DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                          DWRITE_PARAGRAPH_ALIGNMENT verticalAlign = DWRITE_PARAGRAPH_ALIGNMENT_NEAR, bool cache = true,
                          bool hyphen = false, bool wrap = false) override;
    virtual Vec2 getTextSize(std::wstring const& text, Renderer::FontSelection font, float size,
                             bool trailingWhitespace = true, bool cache = true,
                             std::optional<Vec2> bounds = std::nullopt, bool wrap = false) override;
    virtual RectF getTextRect(std::wstring const& text, Renderer::FontSelection font, float size, float pad = 0.f,
    ...
    void drawTextClipped(RectF const& clipRect, RectF const& textRect, std::wstring const& text,
                         ...
                         bool cache = false, bool wrap = false);
    ...
    /// Largest font size (<= maxSize) whose glyphs fully fit inside bounds. Never ellipsizes.
    [[nodiscard]] float fitTextSize(std::wstring const& text, Renderer::FontSelection font, RectF const& bounds,
                                    float maxSize, float minSize = 5.f);
    /// Draws a single line auto-shrunk so every glyph fits inside textRect.
    void drawAutoFitted(RectF const& textRect, std::wstring const& text, d2d::Color const& color,
                        Renderer::FontSelection font, float maxSize,
                        DWRITE_TEXT_ALIGNMENT alignment = DWRITE_TEXT_ALIGNMENT_LEADING,
                        DWRITE_PARAGRAPH_ALIGNMENT verticalAlignment = DWRITE_PARAGRAPH_ALIGNMENT_CENTER,
                        float minSize = 5.f);
};
...
public:
    ...
    void fillTriangle(Vec2 const& a, Vec2 const& b, Vec2 const& c, d2d::Color const& col) override;
    virtual void fillRectangle(RectF const& rect, d2d::Color const& color) override;
    virtual void drawRectangle(RectF const& rect, d2d::Color const& color, float lineThickness = 1.f) override;
    virtual void fillRoundedRectangle(RectF const& rect, d2d::Color const& color, float radius = 10.f) override;
    virtual void drawRoundedRectangle(RectF, d2d::Color const& color, float radius = 10.f, float lineThickness = 1.f,
