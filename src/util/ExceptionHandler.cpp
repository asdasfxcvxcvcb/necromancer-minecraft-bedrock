...
namespace {
    ...
    constexpr std::string_view crashReportTitle = "Necromancer Client Crash Report";
    ...
    std::filesystem::path LogsPath() {
        return util::GetNecromancerPath() / "Logs";
    }
    ...
    std::filesystem::path DailyLogPath() {
        ...
        std::ostringstream oss;
        oss << "Necromancer-" << std::put_time(&now, "%Y-%m-%d") << ".log";
    ...
    HMODULE GetNecromancerModule() {
        HMODULE module = nullptr;
        GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                           reinterpret_cast<LPCWSTR>(&GetNecromancerModule), &module);
    ...
    std::wstring BuildSymbolPath() {
        ...
        auto necromancerModulePath = GetModuleFilePath(GetNecromancerModule());
        AddSymbolPathPart(symbolPath, necromancerModulePath.parent_path());
        AddSymbolPathPart(symbolPath, util::GetNecromancerPath());
    ...
    void EnsureNecromancerModuleLoaded(HANDLE process) {
        auto module = GetNecromancerModule();
    ...
    bool EnsureSymbolsLocked() {
        ...
        if (SymInitializeW(process, symbolPath.empty() ? nullptr : symbolPath.c_str(), TRUE)) {
            ...
            EnsureNecromancerModuleLoaded(process);
            return true;
        }
        ...
        if (error == ERROR_INVALID_PARAMETER) {
            ...
            // keep using that session and update the search path for NecromancerDebug.pdb.
...
#else
#error Unsupported architecture for NecromancerDebug stack traces.
#endif
    ...
    int HandleSehException(EXCEPTION_POINTERS* exceptionInfo, char const* reason) noexcept {
        ...
        try {
            DebugExceptionHandler::WriteCrashReport(exceptionInfo,
                                                    reason ? reason : "Caught SEH exception at Necromancer native boundary");
        } catch (...) {}
...
std::filesystem::path DebugExceptionHandler::WriteCrashReport(EXCEPTION_POINTERS* exceptionInfo,
    ...
    std::string baseName =
        std::format("NecromancerNightlyCrash-{}", MakeTimestamp(true), GetCurrentProcessId(), GetCurrentThreadId());
#elif defined(NECROMANCER_DEBUG)
    std::string baseName =
        std::format("NecromancerDebugCrash-{}", MakeTimestamp(true), GetCurrentProcessId(), GetCurrentThreadId());
#else
    std::string baseName =
        std::format("NecromancerCrash-{}", MakeTimestamp(true), GetCurrentProcessId(), GetCurrentThreadId());
#endif
    ...
    auto necromancerModulePath = GetModuleFilePath(GetNecromancerModule());
    if (!necromancerModulePath.empty()) {
        report << "Necromancer Module: " << PathToUtf8(necromancerModulePath) << "\n";
    }
...
void LogExceptionDetails(const std::exception& e) {
    ...
    DebugExceptionHandler::WriteCrashReport(&exceptionInfo,
                                            std::format("Caught C++ exception at Necromancer error boundary: {}", e.what()));
}
