...
namespace {
    ...
    std::wstring selectedTextLocale() {
        try {
            auto const& languages = Necromancer::get().getL10nData().getLanguages();
            int selectedLanguage = Necromancer::get().getSelectedLanguage();
...
bool Renderer::init(IDXGISwapChain* chain) {
    ...
    if (forceDX11 || !gameDevice12) {
        ...
        isDX11 = false;
        bufferCount = querySwapChainBufferCount(3);
...
#pragma warning(pop)
    ...
    uint32_t idx = getTargetIndex();
    if (idx == UINT32_MAX) {
        Logger::Warn("Renderer init: no valid render target after buffer creation");
        return false;
    }
...
void Renderer::releaseSwapChainTargets() {
    if (d2dCtx) d2dCtx->SetTarget(nullptr);
...
int Renderer::querySwapChainBufferCount(int fallback) const {
    if (!swapChain4) return fallback;

    DXGI_SWAP_CHAIN_DESC desc = {};
    if (FAILED(swapChain4->GetDesc(&desc)) || desc.BufferCount == 0) {
        return fallback;
    }
    return static_cast<int>(desc.BufferCount);
}
...
void Renderer::createTextFormats() {
    ...
    auto collectionFor = [this](std::wstring const& family) -> IDWriteFontCollection* {
        if (!embeddedFonts.isReady()) return nullptr;
        if (family != EmbeddedFontCollection::familyName()) return nullptr;
        return embeddedFonts.get();
    };

    IDWriteFontCollection* primaryCollection = collectionFor(fontFamily);
    IDWriteFontCollection* secondaryCollection = collectionFor(fontFamily2);

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), primaryCollection, DWRITE_FONT_WEIGHT_NORMAL,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primaryFont.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), primaryCollection, DWRITE_FONT_WEIGHT_SEMI_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primarySemilight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), primaryCollection, DWRITE_FONT_WEIGHT_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primaryLight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), secondaryCollection, DWRITE_FONT_WEIGHT_NORMAL,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->secondaryFont.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), secondaryCollection, DWRITE_FONT_WEIGHT_SEMI_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->secondarySemilight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), secondaryCollection, DWRITE_FONT_WEIGHT_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
...
void Renderer::createDeviceIndependentResources() {
    ...
    if (!embeddedFonts.isReady() && !embeddedFonts.create(dWriteFactory.Get())) {
        Logger::Warn("Embedded Minecraft font unavailable, falling back to system fonts");
    }

    if (!pixelFontParams) {
        if (FAILED(dWriteFactory->CreateCustomRenderingParams(1.8f, 0.f, 0.f, DWRITE_PIXEL_GEOMETRY_FLAT,
                                                              DWRITE_RENDERING_MODE_NATURAL,
                                                              pixelFontParams.GetAddressOf()))) {
            pixelFontParams = nullptr;
        }
    }

    if (!primaryFont) createTextFormats();
...
ComPtr<IDWriteTextLayout> Renderer::getLayout(IDWriteTextFormat* fmt, std::wstring const& str, bool cache) {
    ...
    if (cache) {
        ...
        if (it != cachedLayouts.end() && it->second->fmt == fmt) {
            layoutLru.splice(layoutLru.begin(), layoutLru, it->second);
    ...
    auto existing = cachedLayouts.find(hash);
    if (existing != cachedLayouts.end()) {
        existing->second->fmt = fmt;
        existing->second->layout = layout;
        layoutLru.splice(layoutLru.begin(), layoutLru, existing->second);
        return layout;
    }

    layoutLru.push_front({ hash, fmt, layout });
    cachedLayouts[hash] = layoutLru.begin();

    while (cachedLayouts.size() > maxCachedLayouts) {
        auto& victim = layoutLru.back();
        cachedLayouts.erase(victim.key);
        layoutLru.pop_back();
    }
