...
void Asset::load(IWICImagingFactory* factory, ID2D1DeviceContext* dc) {
    ...
    const auto resourceSize = resource.size();
    if (resourceSize > std::numeric_limits<UINT>::max()) {
        return;
    }

    auto stream = ComPtr<IStream>(
        SHCreateMemStream(reinterpret_cast<const BYTE*>(resource.begin()), static_cast<UINT>(resourceSize)));
