...
namespace {
    bool isRightToLeftText() {
        try {
            return Necromancer::get().getL10nData().isSelectedLanguageRightToLeft();
        } catch (...) {
            return false;
        }
    }
...
void TextBox::setSelected(bool b) {
    ...
    if (b) {
        ...
        try {
            Necromancer::getKeyboard().releaseMovementKeys();
        } catch (...) {
        }
    }
}
...
void TextBox::onChar(wchar_t character) {
    if (character == '\b' && text.size() > 0) {
        ...
        if (numericOnly) {
            bool allowed = (character >= L'0' && character <= L'9') || character == L'.' || character == L'-';
            if (!allowed) return;
        }
