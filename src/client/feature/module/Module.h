...
public:
    ...
    explicit Module(std::string const& name, std::wstring const& displayName, std::wstring const& description,
        ...
        key = KeyValue(keybind == nokeybind ? 0 : keybind);
    ...
    virtual bool showToggle() { return true; }
    bool shouldListen() override { return this->isEnabled(); };
    char const* profilerName() override { return modName.c_str(); }
    ...
    std::shared_ptr<Setting> addSetting(std::string const& internalName, LocalizedString const& displayName,
                                        LocalizedString const& desc, ValueType& val,
                                        Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, std::wstring const& displayNameText,
                                            std::wstring const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, LocalizedString const& displayNameText,
                                            std::wstring const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, std::wstring const& displayNameText,
    ...
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, LocalizedString const& displayNameText,
                                            LocalizedString const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSliderSetting(std::string const& internalName, std::wstring const& displayNameText,
    ...
    std::shared_ptr<Setting> addSliderSetting(std::string const& internalName, LocalizedString const& displayNameText,
                                              std::wstring const& desc, ValueType& val, ValueType min, ValueType max,
                                              ValueType interval, Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSliderSetting(std::string const& internalName, std::wstring const& displayNameText,
