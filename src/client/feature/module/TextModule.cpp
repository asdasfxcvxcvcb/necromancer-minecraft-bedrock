...
void TextModule::onInit() {
    ...
    auto customSizeSetting = addSetting("customSize", LocalizeString::get("client.textmodule.props.customSize.name"),
               LocalizeString::get("client.textmodule.props.customSize.desc"), customSize, "fillBg"_istrue);
    customSizeSetting->visible = false;
    addSliderSetting("bgX", LocalizeString::get("client.textmodule.props.bgX.name"),
                     ...
                     FloatValue(2.5f), "fillBg"_istrue);
    ...
    addEnumSetting("alignmnet", LocalizeString::get("client.textmodule.props.alignment.name"),
                   ...
                   "fillBg"_istrue || "showOutline"_istrue);
    ...
    addSliderSetting("radius", LocalizeString::get("client.textmodule.props.radius.name"), L"", radius, FloatValue(0.f),
                     FloatValue(10.f), FloatValue(1.f), "fillBg"_istrue || "showOutline"_istrue);

    settings->forEach([](std::shared_ptr<Setting> s) {
        if (s->name() == "prefix") s->visible = false;
...
void TextModule::render(DrawUtil& dc, bool isDefault, bool inEditor) {
    ...
    if (std::get<BoolValue>(fillBg) || std::get<BoolValue>(customSize)) {
    ...
    // dc.fillRoundedRectangle(rc, Necromancer::get().getHUDBlurBrush(), rad);
    dc.flush();
}
