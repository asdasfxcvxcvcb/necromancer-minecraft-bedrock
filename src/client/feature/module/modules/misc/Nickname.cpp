...
void Nickname::onRenderNameTag(Event& evG) {
    ...
    std::string nicknameText = util::WStrToStr(std::get<TextValue>(this->nickname).str);

    if (nicknameText.empty() || !SDK::ClientInstance::get()->getLocalPlayer()) {
        return;
    }
    ...
    replaceAll(currentNametag, originalName, nicknameText);
}
