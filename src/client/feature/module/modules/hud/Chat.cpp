...
Chat::Chat()
    ...
    // listen<NecromancerClientMessageEvent>((EventListenerFunc)&Chat::onNecromancerMessage);
...
void Chat::render(DrawUtil& ctx, bool isDefault, bool inEditor) {
    ...
    for (auto& msg : messages) {
        if (tm - msg.timeCreated >= (displayDuration + messageAnimDuration)) {
            messages.pop_back();
        }
    }
    ...
    for (size_t i = 0; i < std::min(messages.size(), static_cast<size_t>(maxMessages)); i++) {
        ...
        auto textWrap = [](DrawUtil& util, std::wstring txt, float tSize, float width) -> std::wstring {
            ...
            for (size_t i = retIndex; i < work.size(); i++) {
                float size = util.getTextSize(work.substr(0, i), Renderer::FontSelection::PrimaryRegular, tSize, true, false).x;
                if (size > width) {
...
void Chat::onNecromancerMessage(Event& evG) {
    auto& ev = reinterpret_cast<NecromancerClientMessageEvent&>(evG);
    addMessage(ev.getMessage());
}
