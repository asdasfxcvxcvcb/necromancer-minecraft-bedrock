...
void CPSCounter::onInit() {
    TextModule::onInit();

    settings->forEach([](std::shared_ptr<Setting> s) {
        if (s->name() == "prefix") s->visible = true;
        else if (s->name() == "suffix") s->visible = true;
    });
}
...
std::wstringstream CPSCounter::text(bool isDefault, bool inEditor) {
    ...
    case 0:
        wss << Necromancer::get().getTimings().getCPSL();
        break;
    case 1:
        wss << Necromancer::get().getTimings().getCPSR();
        break;
    case 2:
        wss << Necromancer::get().getTimings().getCPSL() << " | " << Necromancer::get().getTimings().getCPSR();
        break;
    }
    return wss;
}
