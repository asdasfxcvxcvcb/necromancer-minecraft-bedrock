...
TabList::TabList()
    : Module("PlayerList", LocalizeString::get("client.module.tabList.name"),
             LocalizeString::get("client.module.tabList.desc"), HUD, 0) {
    ...
    addSetting("bgColor", LocalizeString::get("client.module.tabList.bgColor.name"),
               LocalizeString::get("client.module.tabList.bgColor.desc"), bgCol, "fillBg"_istrue);
...
bool TabList::skinKeyMatches(PlayerHeadSkinKey const& skinKey, SDK::SerializedSkinRef const& skin,
                             SDK::SkinImage const& image) const {
    auto id = skin.getId();

    if (id && skinKey.id != *id) return false;
    if (!id && !skinKey.id.empty()) return false;

    if (skinKey.width != image.width) return false;
    if (skinKey.height != image.height) return false;

    if (skinKey.bytes != reinterpret_cast<uintptr_t>(image.bytes.data())) return false;
    if (skinKey.byteCount != image.bytes.size()) return false;
...
TabList::PlayerHeadSkinKey TabList::makeSkinKey(SDK::SerializedSkinRef const& skin, SDK::SkinImage const& image) const {
    ...
    skinKey.width = image.width;
    skinKey.height = image.height;
    skinKey.bytes = reinterpret_cast<uintptr_t>(image.bytes.data());
    skinKey.byteCount = image.bytes.size();
    return skinKey;
}

std::string TabList::getRowName(SDK::PlayerListEntry& entry) const {
    if (auto formattedName = Necromancer::get().getNameTagCache().getFormattedPlayerName(entry.name)) return *formattedName;
    return entry.name;
}
...
void TabList::onRenderNameTag(Event& evG) {
    ...
    if (!tag || !Necromancer::get().getNameTagCache().hasFormatCode(*tag)) return;
    ...
    if (Necromancer::get().getNameTagCache().recordRenderedNameTag(*tag, cachedActivePlayerNames)) {
        rowsDirty = true;
    }
}
...
void TabList::onRenderLayer(Event& evG) {
    ...
    MCDrawUtil dc { ev.getUIRenderContext(), Necromancer::get().getFont() };
    ...
    // Heads early-return without a render context, so only pay for the live
    // entry lookup map when it can actually be used. string_view keys point at
    // the game's player-list names, which are stable for the duration of this
    // frame, so no per-name string copy is needed.
    if (dc.renderCtx) {
        std::unordered_map<std::string_view, SDK::PlayerListEntry*> currentEntries;
