...
bool ConfigCommand::execute(std::string const label, std::vector<std::string> args) {
    ...
    if (args[0] == "save") {
        if (args.size() < 2) {
            if (Necromancer::getConfigManager().saveCurrentConfig())
        ...
        if (Necromancer::getConfigManager().saveTo(util::StrToWStr(args[1]))) {
            message(
        ...
        if (!Necromancer::getConfigManager().loadUserConfig(util::StrToWStr(args[1]))) {
            message(
