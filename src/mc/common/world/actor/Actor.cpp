...
#include "mc/common/entity/component/OnGroundFlagComponent.h"
...
uint64_t SDK::Actor::getRuntimeID() {
    auto comp = this->tryGetComponent<RuntimeIDComponent>();
...
bool SDK::Actor::isOnGround() {
    return entityContext.enttRegistry.storage<OnGroundFlagComponent>().contains(entityContext.entity);
}

SDK::ItemStack* SDK::Actor::getArmor(int armorSlot) {
    auto* component = this->tryGetComponent<ActorEquipmentComponent>();
    if (!component || !component->armorContainer) return nullptr;
    return component->armorContainer->getItem(armorSlot);
}
